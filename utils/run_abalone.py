...
from generate_data import load_abalone_regression_data
import re
...
# Argumentparser
parser = argparse.ArgumentParser(description="Run regression model on Abalone dataset.")
...
# Last inn data
X_train, X_test, y_train, y_test = load_abalone_regression_data(standardized=args.standardize, frac=1.0)
N, p = X_train.shape
seed = 42
data_type = "abalone"
if args.standardize:
    config_name = f"abalone_N{N}_p{p}_standardized"
else:     
    config_name = f"abalone_N{N}_p{p}"
