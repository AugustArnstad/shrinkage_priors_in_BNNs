// =====================
// Bayesian Neural Network with Spike-and-Slab Beta Prior (Input Layer Only)
// =====================
...
parameters {
  array[H] vector<lower=0, upper=1>[P] lambda;
  matrix[P, H] slab_raw;
...
transformed parameters {
  ...
  for (j in 1:H)
    for (i in 1:P)
      data_to_hidden[j][i] = lambda[j][i] * slab_raw[i, j] * c;
  ...
  matrix[N, output_nodes] output = nn_predict(X, data_to_hidden_mat, hidden_to_hidden,
